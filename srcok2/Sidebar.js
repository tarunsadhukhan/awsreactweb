...
*/
      axios.get('http://localhost:5000/api/menu')
  .then(response => {
    console.log('Menu Data:', response.data); // Check if the data is structured correctly
    setMenuData(buildMenuHierarchy(response.data));
  })
  .catch(error => console.error('Error fetching the menu:', error));
...
const buildMenuHierarchy = (data) => {
  ...
  data.forEach(item => {
    if (item.parent_id === 0) {
      ...
      // Ensure parent exists before pushing children
